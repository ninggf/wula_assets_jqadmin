...
layui.define(['jquery', 'layer', 'toastr'], function (exports) {
	...
	(function ($, layer, wulaui) {
		...
		var getMsg = function getMsg(rq) {
			var t = rq.responseText || rq.statusText;
		...
		$('body').on('click', 'a[data-ajax]', doAjaxRequest).on('click', 'button[data-ajax]', doAjaxRequest).on('submit', 'form[data-ajax]', doAjaxRequest).on('change', 'select[data-ajax]', doAjaxRequest).on('change', 'input[data-ajax]', doAjaxRequest).on('change', 'textarea[data-ajax]', doAjaxRequest).on('ajax.send', '[data-loading]', function (e) {
			e.stopPropagation();
			var me = e.element;
	...
	(function ($) {
		$.fn.wulauiDatepicker = function () {
			return $(this).each(function (i, e) {
				var $this = $(e),
				    inited = $this.data('DatepickerObj');
				if (!inited) {
		...
		$('body').on('wulaui.widgets.init', '.wulaui', function (e) {
			e.stopPropagation();
			var that = $(this).find('[data-datepicker]');
			if (that.length > 0) {
